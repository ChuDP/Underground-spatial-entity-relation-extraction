...
##训练数据标注
#################实体标注及实体位置标注##################
    inputs = open('Data/train_data_pre_100.txt', 'r', encoding='utf-8')
    outputs = open('result/train_data_pre_100_entity.txt', 'w', encoding='utf-8')
...
#测试数据标注
################实体标注及实体位置标注##################
    inputs = open('Data/test_data_pre_100.txt', 'r', encoding='utf-8')
    outputs = open('result/test_data_pre_100_entity.txt', 'w', encoding='utf-8')
    entitycankaoku = '关系提取实体库_去重_可扩展.txt'
    relationku = '关系提取关系词库_quchong.txt'
    biaozhu_entity(entitycankaoku, inputs,outputs)
